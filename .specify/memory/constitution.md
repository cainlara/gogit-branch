<!--
Sync Impact Report
- Version change: 1.2.0 → 1.3.0 (MINOR: `clone` added to the sanctioned network-activity
  list and to the Development Workflow subcommand dispatch list; no numbered principle
  removed or redefined)
- Modified principles:
  - Additional Constraints (network policy) → command list extended: "push, pull, fetch,
    clone, and — in switch — two refreshes"; clone performs one network operation (a single
    `git clone --recurse-submodules` fetching the repository plus its submodules into the
    launch pwd) and no others
  - Development Workflow → subcommand dispatch list gains `clone`/`cl`
- Added sections: none
- Removed sections: none
- Modified sections: none other
...
- Follow-up TODOs: none (placeholders deferred; RATIFICATION_DATE unchanged: 2024-07-22)
...
- All network activity is performed by `git` itself and only where a command requires it:
  `push`, `pull`, `fetch`, `clone`, and — in `switch` — two refreshes: **(a)** the pre-listing refresh
...
- One subcommand = one file under `execution/`, with a single exported entry point wired
  ...
  `batch-delete`/`bd`, `create`/`c`, `clone`/`cl`, `status`/`st`, `push`/`p`, `pull`/`pl`,
  `log`/`l`, `reset`/`r`, `version`/`v`, `help`/`h`).
...
**Version**: 1.3.0 | **Ratified**: 2024-07-22 | **Last Amended**: 2026-09-25
