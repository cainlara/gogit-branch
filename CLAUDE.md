...
- **`execution/*.go`** — one file per subcommand, each exposing a single entry point called from `main.go` (`ListCurrentBranches`, `BrowseAndSwitchBranches`, `ListAndDeleteBranch`, `ListAndDeleteBranches`, `CreateAndSwitchBranch`, `ShowStatusAndOperate`, `PushCurrentBranch`, `PullCurrentBranch`, `ShowLog`, `ResetToLatestCommit`, `PrintHelp`, `ShowVersion`). `log`/`l` and `reset`/`r` join `create`/`c` as the three subcommands that accept an optional positional CLI argument (a limit, or the `--hard` flag, respectively); `main.go`'s argument-count guard explicitly names all three as the only exceptions — `pull`/`pl` deliberately takes no arguments and is therefore rejected like any non-listed command when extra args are passed. `common.go` holds the shared `promptui.Select` wiring used by both single-select (switch/delete) and multi-select (batch-delete) flows, plus the emoji/banner constants. `create.go`'s `CreateAndSwitchBranch` is the exception to the `Select`-based UX: it takes a new branch name via a freeform `promptui.Prompt` instead, since there is nothing existing to select from — unless the caller already supplied the name as a CLI argument (`create <name>` / `c <name>`), in which case `main.go` passes it through as a non-nil `*string` and the prompt is skipped entirely. A supplied name is trimmed and validated (rejecting empty or dash-led values) in `create.go`'s `validateBranchName`; the interactive-prompt path is intentionally left unvalidated/unchanged. `status.go`'s `ShowStatusAndOperate` renders three sections (tracked files, untracked files, options) from `core.GitClient.Status()` and reads a single raw key press via `golang.org/x/term` (no Enter required) rather than using `promptui.Select`.
...
`delete` and `batch-delete` both call `git branch -D` (force delete, ignores unmerged-changes safety) and always prompt for an explicit `yes`/`y` confirmation before invoking `core.GitClient`. Preserve this confirm-before-force-delete pattern when touching these flows.

`delete`, `batch-delete`, and `reset`'s `confirmXxx` functions all build their own label text, then call a shared `confirmYesNo(label string) bool` in `execution/common.go` rather than constructing a `promptui.Prompt{IsConfirm: true}` directly. `IsConfirm: true` is intentionally avoided: promptui's own built-in accept check only ever treats an exact `y` (or an empty Enter against a `y` `Default`) as accepted, unconditionally force-rendering its rejection template for anything else — including `yes`, which every affected prompt's own label explicitly invites and which this tool has always separately treated as accepted. That mismatch let a prompt visually render as rejected while the destructive action still proceeded. `confirmYesNo` closes the gap by owning both decisions itself: it normalizes the answer case-insensitively and whitespace-trimmed via `isAffirmativeAnswer` (accepting exactly `y`/`yes`), prints an explicit accepted-confirmation line only when accepted, and returns that same boolean — so the rendered state and the actual outcome can no longer disagree. Each call site keeps its own distinct decline message (`"Deletion aborted"`, `"Reset aborted"`, batch-delete's equivalent); `confirmYesNo` does not print anything on decline, to avoid duplicating that messaging. `batch-delete`'s confirmation label additionally passes through `summarizeBranchNamesForConfirm` (in `execution/batch_delete.go`), which caps the displayed branch names at 5 plus an "and N more" suffix — the label is otherwise unbounded and data-driven (every selected branch name), which risked wrapping and redraw corruption on narrower terminals; the full selection is still passed to `gitClient.DeleteBranches` regardless of how many names the label shows.
...
`push`/`p` (`core.GitClient.Push`) always attempts a plain `git push` first. Only if that fails with git's own, stable "has no upstream branch" message does it resolve the current branch name (`git rev-parse --abbrev-ref HEAD`) and retry once as `git push --set-upstream origin <branch>`, so a brand-new local branch publishes in a single command instead of surfacing that error to the user. Every other push failure (no remote configured, a non-fast-forward rejection, detached HEAD, etc.) is returned untouched — `Push` never force-pushes or otherwise bypasses git's safety checks. Because a rejected `git push`'s combined output can start with a `To <remote>` progress line before the `error:`/`fatal:` line (unlike the checkout/log/reset family, whose failures emit that prefix as their very first line), this error wrapping checks for the prefix anywhere in the output (`strings.Contains`) rather than only at the start (`strings.HasPrefix`) — it lives in the shared `outputError` helper used by `Push`, `Fetch`, and `Pull`, which additionally falls back to carrying marker-free output verbatim when it is non-empty (`git pull`'s no-upstream and detached-HEAD messages have no `error:`/`fatal:` prefix at all), and only returns the raw command error when there was no output.

### Pull command fetch-then-pull ordering

`pull`/`pl` (`execution.PullCurrentBranch` → `core.GitClient.Fetch` then `core.GitClient.Pull`) always runs a plain `git fetch` first and returns immediately if that fetch fails, so the pull step is unreachable after a failed fetch (FR-002/FR-003 — one visible "Fetching remote updates..." line precedes any result). On success it classifies the outcome by checking the combined pull output for the `ALREADY_UP_TO_DATE_MARKER` (`"Already up to date"`, substring-matched so the legacy hyphenated spelling matches too) and prints either the updated or the up-to-date success line. `Pull` runs a plain `git pull` with no `--force`/`--rebase`, so git's own refusals (divergent-branches reconciliation required, local-changes overwrite protection, detached HEAD, no upstream) surface untouched and the command never discards uncommitted work. One environment quirk validated during implementation: `git fetch` with **no remote configured** exits 0 silently (a no-op), so that scenario's error surfaces at the pull step as git's "no tracking information" message rather than at fetch. `git remote remove` also prunes `branch.<name>.remote`/`merge`, so restoring a removed remote requires re-setting the upstream (`git fetch` + `git branch --set-upstream-to`), not just `git remote add`.
